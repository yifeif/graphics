...
static tensorflow::Status GetVariablesRank(
    ::tensorflow::shape_inference::InferenceContext* c, tensorflow::int32* rank) {
...
class RasterizeOp : public tensorflow::OpKernel {
 ...
 private:
  ...
  tensorflow::Status RenderImage(
      tensorflow::OpKernelContext* context,
      std::unique_ptr<RasterizerWithContext>& rasterizer, tensorflow::int64 image_size,
      float* image_data);
...
tensorflow::Status RasterizeOp::RenderImage(
    tensorflow::OpKernelContext* context,
    std::unique_ptr<RasterizerWithContext>& rasterizer, const tensorflow::int64 image_size,
    float* image_data) {
...
tensorflow::Status RasterizeOp::SetVariables(
  ...
  for (int index = 0; index < variable_names_.size(); ++index) {
    ...
    if (kind == "mat") {
      ...
      const tensorflow::int32 buffer_length = value_shape.dim_size(value_shape.dims() - 1);
