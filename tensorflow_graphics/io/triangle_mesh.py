...
# TODO(segaviv): Add tests and extract to a new CL. We need to decide first if
# we want to use trimehs or a different library for loading the meshes.
class GfileResolver(trimesh.visual.resolvers.Resolver):
  ...
  def get(self, name):
    with tf.io.gfile.GFile(os.path.join(self.directory, name.strip()),
                           'rb') as f:
      data = f.read()
    return data
...
def load(file_obj, file_type=None, **kwargs):
  ...
  raise ValueError('file_obj should be either a GFile or a string')
